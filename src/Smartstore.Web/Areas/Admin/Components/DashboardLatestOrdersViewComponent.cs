...
{
    public class DashboardLatestOrdersViewComponent(CustomerSettings customerSettings)
        : SmartViewComponent
    {
            ...
            foreach (var order in allOrders)
            {
                model.LatestOrders.Add(
                    new()
                    {
                        ...
                        CustomerDisplayName = order.Customer.FormatUserName(
                            _customerSettings,
                            T,
                            false,
                            true
                        ...
                        TotalAmount = Services.CurrencyService.CreateMoney(
                            order.OrderTotal,
                            primaryCurrency
                        ),
                        Created = Services
                            .DateTimeHelper.ConvertToUserTime(order.CreatedOnUtc, DateTimeKind.Utc)
                            .ToString("g"),
        ...
        private async Task<List<Order>> GetOrdersByStatus(OrderStatus status, int count)
        {
            ...
            var isMerchant = await Services.DbContext.CustomerRoleMappings.AnyAsync(m =>
                m.CustomerId == currentUser.Id && m.CustomerRole.SystemName == "Merchant"
            );

            IQueryable<Order> query = Services
                .DbContext.Orders.ApplyCustomerFilter(authorizedStoreIds)
                .AsNoTracking()
                .AsSplitQuery();

            if (isMerchant)
            {
                var merchantProductIds = await Services
                    .DbContext.GenericAttributes.Where(a =>
                        a.KeyGroup == "Product"
            ...
            return orders.Where(x => x.OrderStatus == status).Take(count).ToList();
        }
    }
}
